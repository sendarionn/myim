...
- Mozc OSS、TKGJE、myim補完辞書、Cosense拡張辞書を重ねて候補を生成
...
かな漢字変換の主辞書としてMozc OSS辞書を同梱します

同梱辞書はオリジナルのMozc辞書そのものではなく、Mozc OSSの原データからmyimの基本辞書形式へ必要な読みと候補だけを抽出した派生データです
...
- 1読みにつき最大32候補
- かな読みの二分探索で前方一致候補を取得
- 前方一致候補は1回の入力更新につき最大2048件

`kakujuu`から`拡充`、`kangaeru`から`考える`などを変換できます

CC0のTKG Japanese-English Learner’s Dictionaryを学習者向け日英補助辞書として同梱します

同梱辞書はオリジナルのTKGJEそのものではなく、公開索引からmyimの基本辞書形式へローマ字読みと見出し語だけを抽出した派生データです

TKGJEで`速い／早い`のようにスラッシュで併記された別表記は、`速い`と`早い`の独立した変換候補へ分割します
...
両方の辞書はIMのビルド時にアプリ内へコピーされ、ネットワーク接続なしで利用できます
...
Mozc辞書は`Scripts/convert-mozc-dictionary.py`で候補コスト順に変換します

辞書の出典と配布条件は`docs/DICTIONARY_LICENSES.md`を参照してください

## 漢字候補の補完

Mozc OSSとTKGJEにない読みと漢字の組み合わせをmyim補完辞書から追加します

- `kakujuu`から`拡充`を表示
- 上流辞書を更新しても補完辞書を維持
- ユーザー辞書、Cosense拡張辞書、Mozc OSS、TKGJEを補完辞書より優先
- 補完候補をひらがなと片仮名より優先
- 不足例を回帰テストへ追加

## 動詞活用候補

Mozc OSSまたはTKGJEに収録された終止形から活用候補を生成します
...
現在の基本辞書は品詞と活用型を保持していないため、同じ読みを持つ一段動詞と五段動詞を完全には判別できません

今後は活用型を持つ辞書データを補完レイヤーへ追加し、表層的な語尾判定を置き換えます
...
- `YYYYMMDD`などの書式指定を入力してから`Control＋T`を押すと、指定書式の日付または時刻へ置換
- 書式指定は許可したトークンだけを解釈し、通常の文字列は変更しない
